function DataManager_RettMice_EventParticipation
...
if ok
    ...
    pinfo = FindEventSpikingProperties(pinfo,data,cellind);
    %%%update plots
...
function pinfo = FindEventSpikingProperties(pinfo,data,cellind)
%variable to assign
if (~isempty(cellind))
...
for (jjjk = 1:numel(cellind))
    i = cellind(jjjk); 
    disp(['-----> computing firing event participattion (', num2str(jjjk), ' out of ', num2str(numel(cellind)), '): ', pinfo.general.parmfile{i}]);
    ...
    nev = numel(evName);
    pinfo.firing.evtPartRate{i} = NaN*ones(1, nev); pinfo.firing.evtMeanNSpike{i} = NaN*ones(1, nev);
    for (tt = 1:nev)
