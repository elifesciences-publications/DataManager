function DataManager_RettMice_RippleParticipationCoactivity
...
if (~isempty(cellind))
   ...
   if (~isfield(pinfo.firing, 'evtMeanSpike')) pinfo.firing.evtMeanSpike = cell(1, nspike); end
end
for (jjjk = 1:numel(cellind))
    ...
    nev = numel(evName);
    pinfo.firing.evtParteRate{i} = NaN*ones(1, nev); pinfo.firing.evtMeanSpike{i} = NaN*ones(1, nev);
end
