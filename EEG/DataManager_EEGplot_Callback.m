function DataManager_EEGplot_Callback
...
for (k = 1:ngroup)
    ...
    cellname{k} = pinfo.general.eegfile(spikeselectionindex);
end
...
%%val{group, field}{cells} = either single value or vector of values
...
if (strcmp(tagmark, 'function')) %%%if do a function between two variables
    plotdatafunction(val, groupname, fieldname, plotparm, comtype, tagmark, cellname, col);
...
if (strcmp(tagmark, 'hist'))
   ...
   hf = figure('Name', strcat(plotname, '-', tagmark)); hax = axes('Parent', hf, 'NextPlot', 'add'); str = []; stry = [];
   for (i = 1:n)
       if (~isempty(val{i}))
          co{i} = rand(1,3);
          if (plotparm.setlog == 1) | (plotparm.setlog == 4) 
              y = log10(val{i}); stry = strcat('Log10(', tagmark, ')');
          else
              y = val{i}; stry = tagmark;
          end
          Y = histc(y, binvector); if (~strcmp(comtype, 'Individuals')) Y = Y/sum(Y); end
...
elseif (strcmp(tagmark, 'cumu'))
   ...
   for (i = 1:n)
       co{i} = rand(1,3);
       if (plotparm.setlog == 1) | (plotparm.setlog == 4) 
           y = log10(abs(val{i})); stry = strcat('Log10(', tagmark, ')');
       else
           y = val{i}; stry = tagmark;
       end
       Y = histc(y, binvector); if (~strcmp(comtype, 'Individuals')) Y = Y/sum(Y); end
...
function plotdatafunction(val, groupname, fieldname, plotparm, comtype, tagmark, cellname, col)
[ngroup, nfield] = size(val); ncell = numel(val{1,1});
    if (strcmp(comtype, 'Between groups'))
        for (m = 1:nfield) %%%%the earlier variable plotted as x
             for (n = m+1:nfield) %%%%the earlier variable plotted as x
                 XX = cell(1, ngroup); YY = cell(1, ngroup); EE = cell(1, ngroup); str = cell(1, ngroup);
                 for (i = 1:ngroup)
                     allXX = val{i,n}; allYY = val{i,m}; longX = []; longY = [];
                     ...
                     XX{i} = sort(XX{i}); YY{i} = NaN*ones(size(XX{i})); EE{i} = NaN*ones(size(XX{i})); 
                     for (j = 1:numel(XX{i}))
                         iii = find( longX == XX{i}(j) ); ynow = longY(iii); ynow = ynow(ynow>1e-50); 
                         if (~isempty(ynow))
                 ...
                 for (i = 1:ngroup)
                     text('Parent', hax, 'String', str{i}, 'Color', col{i}, 'Units', 'normalized', 'Position', [0.05 0.96-(i-1)*0.04]);
                 end
             end
        end
    elseif (strcmp(comtype, 'Individuals'))
