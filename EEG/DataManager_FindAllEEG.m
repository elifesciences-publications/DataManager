...
for (i = 1:neegdir)
    ...
    if (exist(finalpath, 'dir') == 7) %if final dir exist
        ...
        for (tk = 1:numel(EEGfile))
             ...
             %%%session info
             eeg.general.sessname{neeg} = sess{tk};
             ...
             %%%animal info
             [animname, moreF, moreC] = findanimalname(Ttype, Tfield, Tcontent, TTnow{tk});
             if (~isempty(animname))
...
while 1               %find where header ends
          tline = fgets(fid);
          if (strncmpi(tline, '-ADBitVolts', 10)) %gain = voits per AD value
             [str, ent] = strtok(tline, ' '); ent = ent(2:numel(ent)); gain = str2num(ent);
          end
          if (strncmpi(tline, '-SamplingFrequency', 15)) %gain = voits per AD value
