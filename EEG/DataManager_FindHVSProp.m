...
if ok
   [eeg, eegdata] = DohvsAnalysis(eeg, eegdata, eegind, vv);
end

function [eeg, eegdata] = DohvsAnalysis(eeg, eegdata, eegind, vv)
...
i = eegind(iiik);
if strcmp(eeg.parm.band{i}, 'hvs') %%only do this for filtered EEG traces
    disp(['--------> hvs analysis: ', eeg.general.eegfile{i}]);
    ...
    maxgap = 0.25*eeg.general.freq{i}; %%%now maxgap in datapoints - maxgap within HVS events set as 0.25s (missing 2 cycles at 8Hz)
    ...
    maxdur = eeg.parm.hvsMaxDur(i)*eeg.general.freq{i}; 
    %%%get eeg data
    ...
    elseif (strcmp(ripnorm, 'run'))
        runstd = findrunsessstd(eeg, eegdata, eeg.general.finaldir{i}, 'hvs', eeg.general.recarea{i}); 
        if (~isempty(runstd))
    ...
    if (~isempty(eeg.general.sesslength{i})) && (eeg.general.sesslength{i}>0)
        %%%detect hvss
        hvs = findallhvss(timestamp, dat, pth, sth, mingap, maxgap, minPeakNum, maxdur); %%hvs.startT(i), .peakT, .endT, .dur, .amp
...
function hvs = findallhvss(timestamp, dat, pth, sth, mingap, groupspan, minPeakNum, maxdur) %%hvs.startT(i), .peakT, .endT, .dur, .amp
hvs.startT = []; hvs.peakT = []; hvs.endT = []; hvs.dur = []; hvs.amp = []; hvs.eng = []; 
hvs.minTime = []; hvs.maxTime = []; hvs.minAmp = []; hvs.maxAmp = [];
...
if (~isempty(index))
    ...
    if (~isempty(startindex))
       ...
       hvs = findamp(dat, timestamp, startindex, endindex, pth, minPeakNum, sth);
...
function hvs = findamp(dat, timestamp, startindex, endindex, pth, minPeakNum, sth)
...
for (i = 1:numel(startindex))
     ...
     if (abspeak >= pth)
         ...
         if (numel(find(peakVal>=pth))>= minPeakNum)
             nevent = nevent + 1;
             hvs.amp(nevent) = abspeak; hvs.peakT(nevent) = timenow(peakindex);
