...
pp = {'Enter start threshold (norm count):'; 'Enter peak threshold (norm count):'; 'Enter max gap (s):'};
if (strcmp(mode, 'self'))
    def = {'0.1'; '0.1'; '0.06'};
...
if (~isempty(III))
   pthr = str2num(III{2}); 
   sthr = str2num(III{1}); mgap = str2num(III{3});  
else
    ook = 0;
end
if ook
...
if (ok)
pp = {'Enter peak threshold (std/mV):'; 'Enter start threshold (std/mV):'; 'Enter max gap (s):';...
      'Enter min number of peaks:'; 'Enter max duration (s):'};
if (strcmp(norm, 'self'))
    def = {'6'; '2.5'; '0.5'; '5'; '100'};
elseif (strcmp(norm, 'run'))
    def = {'6'; '2.5'; '0.5'; '5'; '100'};
else
    def = {'0.3'; '0.12'; '0.5'; '5'; '100'};
end
III=inputdlg(pp, 'Parameters for computing HVS properties', 4, def, 'on');
if (~isempty(III))
   pthr = str2num(III{1}); sthr = str2num(III{2}); mgap = str2num(III{3}); mindur = str2num(III{4}); maxdur = str2num(III{5}); 
   nspike = numel(pinfo.general.eegfile);
   ...
   ncell = numel(cellind);
   for (i = 1:ncell) pinfo.parm.hvsNorm{cellind(i)} = norm; end
...
if (ok)
pp = {'Enter peak threshold (std/mV):'; 'Enter start threshold (std/mV):'; 'Enter max gap (s):';...
      'Enter min duration (s):'; 'Enter max duration (s):'; 'Enter max amplitude (std):'};
if (strcmp(norm, 'self'))
    def = {'6'; '2.5'; '0.03'; '0.03'; '0.4'; '25'};
elseif (strcmp(norm, 'run'))
    def = {'6'; '2.5'; '0.03'; '0.03'; '0.4'; '25'};
else
    def = {'0.15'; '0.1'; '0.03'; '0.03'; '0.4'; '0.6'};
end
III=inputdlg(pp, 'Parameters for computing ripple properties', 4, def, 'on');
if (~isempty(III))
   pthr = str2num(III{1}); sthr = str2num(III{2}); mgap = str2num(III{3}); mindur = str2num(III{4}); 
   maxdur = str2num(III{5}); maxamp = str2num(III{6}); 
   nspike = numel(pinfo.general.eegfile);
   ...
   ncell = numel(cellind);
   for (i = 1:ncell) pinfo.parm.rippNorm{cellind(i)} = norm; end
