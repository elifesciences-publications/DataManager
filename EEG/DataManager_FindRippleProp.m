...
if strcmp(eeg.parm.band{i}, 'ripple') %%only do this for filtered EEG traces
    ...
    maxgap = 0.03*eeg.general.freq{i}; %%%now maxgap in datapoints - maxgap within ripple events set as 0.02s (missing 3 cycles at 100Hz)
    mingap = eeg.parm.rippMaxGap(i)*eeg.general.freq{i}; %%%%%%now mingap between ripple events in datapoints
    ...
    %%%detect ripples
    ripp = findallripples(timestamp, dat, pth, sth, mingap, maxgap, mindur, maxdur, maxamp); %%ripp.startT(i), .peakT, .endT, .dur, .amp
...
function ripp = findallripples(timestamp, dat, pth, sth, mingap, groupspan, mindur, maxdur, maxamp) %%ripp.startT(i), .peakT, .endT, .dur, .amp
...
if (~isempty(index))
    ...
    if (~isempty(startindex))
       ...
       ripp = findamp(dat, timestamp, startindex, endindex, pth, maxamp);
...
function ripp = findamp(dat, timestamp, startindex, endindex, pth, maxamp)
...
for (i = 1:numel(startindex))
     ...
     %[pospeak, peakindex] = max(datnow); %%if either positive crossing
     [negpeak, peakindex] = min(datnow); %%if either negative crossing
     %%%%[abspeak, peakindex] = max(abs(datnow)); %%if either positive or negative crossing
     if (negpeak <= -pth) && (negpeak > -maxamp)
     %if (pospeak >= pth)
         nevent = nevent + 1;
         %ripp.amp(nevent) = pospeak; 
         ripp.amp(nevent) = negpeak; 
