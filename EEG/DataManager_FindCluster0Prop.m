...
if (strcmp(SS, 'Yes'))
    ...
    for (i = 1:numel(iii))
        ...
        if (exist(cl0filenow, 'file') == 2)
           ...
           [timepoint, cnt, newmode, ss, mm, ma] = computecl0pwr(sptime, starttime, endtime, parm, eeg, sessid);
           [eeg, eegdata, extra, eegind] = appendcl0file(eeg, eegdata, sessid, cl0filenow, timepoint, cnt, newmode, extra, eegind, ss, mm, ma);
        else
...
if strcmp(eeg.parm.band{i}, 'cluster0') %%only do this for filtered EEG traces
    ...
    cl0 = findallcluster0s(timepoint, cnt, sth, maxgap); %%cl0.startT(i), .peakT, .endT, .dur, .amp, .eng, .sdur; 
    %%%session variables
...
function [timepoint, cnt, newmode, ss, mm, ma] = computecl0pwr(sptime, starttime, endtime, parm, eeg, sessid)
...
if (strcmp(parm.normmode, 'self'))
   ss = std(cnt); mm = mean(cnt); ma = max(cnt);
   disp(['---------------> self std: ', num2str(ss), '; mean:', num2str(mm), '; max:', num2str(ma)]);
elseif (strcmp(parm.normmode, 'run'))
   ...
   if (~isempty(runcnt))
       ss = std(runcnt); mm = mean(runcnt); ma = max(runcnt);
       disp(['---------------> run std: ', num2str(ss), '; mean:', num2str(mm), '; max:', num2str(ma)]);
   else
...
if (strncmpi(parm.normmeth, 'Z-transform', 3))
    cnt = cnt - mm; if (ss~=0) cnt = cnt/ss; end
elseif (strncmpi(parm.normmeth, '0-1', 3)) 
    if (ma > 0) cnt = cnt/ma; end
elseif (strncmpi(parm.normmeth, 'mean-subtraction', 3)) 
    cnt = cnt-mm;
end

function [eeg, eegdata, extra, eegind] = appendcl0file(eeg, eegdata, index, eegfilenow, timepoint, cnt, newmode, extra, eegind, ss, mm, ma)
if strcmp(eeg.parm.band{index}, 'cluster0') %%%if the analyzed file itself is already a cluster0 files, just replace the parameters
    extra.ss{index} = ss; extra.mm{index} = mm; extra.ma{index} = ma;
...
extra.ss{nfile+1} = ss; extra.mm{nfile+1} = mm; extra.ma{nfile+1} = ma; 
...
function cl0 = findallcluster0s(timestamp, dat, sth, maxgap) %%cl0.startT(i), .peakT, .endT, .dur, .amp, .eng, .sdur; 
...
if (~isempty(index))
    ...
    cl0 = findamp(dat, timestamp, startindex, endindex);
...
function cl0 = findamp(dat, timestamp, startindex, endindex)
