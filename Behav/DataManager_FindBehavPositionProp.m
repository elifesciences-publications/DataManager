...
if (~isfield(behav.behavior, 'evtMeanDur')) behav.behavior.evtMeanDur = cell(1, nsess); end %number: mean traj length (s), averaged across all laps and all trajectories
if (~isfield(behav.behavior, 'evtMeanSpeed')) behav.behavior.evtMeanSpeed = cell(1, nsess); end%number: mean speed (pixel/s), averaged across all laps and all trajectories
...
for ttp = 1:numel(sessind)
    ...
    behav.behavior.sessDur{i} = etime-stime; behav.behavior.sessMeanSpeed{i} = mean(AllVel);
    
    %%compute event variables
    for (j = 1:numel(evname))
        evstart = evTimes{j}.start; evend = evTimes{j}.ent; nev = numel(evstart);
        behav.behavior.evtNum{i}(j) = nev; 
        behav.behavior.evtMeanDur{i}(j) = mean(evend-evstart); behav.behavior.evtTotDur{i}(j) = sum(evend-evstart);
        ...
        if ~isempty(posXX)
            behav.behavior.evtMeanSpeed{i}(j) = mean(AllVel(allposind)); 
        else
            behav.behavior.evtMeanSpeed{i}(j) = NaN; 
        end
        ...
        if (strcmp(evType{j}, 'run'))  %if this is run event
                ...
                alloccu = []; occunow = cell(1, numel(evstart)); xnowexit = 0;  %disp(['------- get in now: ', evname{j}]);
                for (tt = 1:nev)
                    ...
                    if ~isempty(xnow)
                       ...
                       [occunow{tt}, xbin, allX, allY, pout] = OccupancyTimeEps_binsize(joint, xnow, ynow, framerate, s1dbinsize, 0, trajstart, trajend);
...
%                 end
                ...
                aa = bhdata.event.LapTrajleng{i}{j}; behav.behavior.evtMeanTrajleng{i}(j) = mean(aa(~isnan(aa)));
                end
         end
    end
end
