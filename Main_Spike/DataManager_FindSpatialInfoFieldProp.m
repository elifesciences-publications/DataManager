...
for (jjjk = 1:numel(cellind))
    ...
    for (tt = 1:nsess)
        ...
        if (numel(posid) ~= 1)
            ...
            [D2ratenow{tt}, n2Dact(tt)] = get2Dratemaps(spiketime, XYbin{tt}, gridoccup{tt}, postime, XX, YY, smParm);
        end
    end
    ...
    for (tt = 1:nsess) 
        ...
        if (nx > 1) && (ny > 1)
            baserate(tt) = computebaserate(D2ratenow{tt}, base2DrateN);
            pf = find2Dfieldprop(D2ratenow{tt}, XYbin{tt}, threshold2Drate, max2Dgap, min2Dpeakrate, baserate(tt));
            for (j = 1:numel(pf))
             nfield = nfield + 1;
    ...
    pinfo.field.PF2DBaseRate{i} = baserate; 
    pinfo.field.PF2DNfield{i} = nfield; disp(['-----------> number of 2D fields identified: ', num2str(nfield)]);
            ...
            if (numel(posid)~=1)|(numel(evid)~=1)
               ...
               nevt = numel(evTime{j}.start);
                if (nevt<minlap)
                    ...
                    xjoint{j} = bhdata.event.Xjoint{posid}{evid};
                    ...
                    [D1ratenow{j}, n1Dact(j)] = getlinearmap(spiketime, evTime{j}, lappostime, lapx, xbin{j}, occutime{j}, smParm);
                end
            end
        end
    end
    ...
    for (tt = 1:numel(dirpairs)) %%%directionality: shift rate maps by # bins, then compute the overlap
        ...
        if (~isempty(rate1)) && (~isempty(rate2))
           ...
           %%%% revision: decide to change to a different alignment
           xjoint1 = xjoint{dirpairs{tt}(1)}; xjoint2 = xjoint{dirpairs{tt}(2)}; 
           [rate1, rate2, x] = realignjointxx(xjoint1, xjoint2, rate1, rate2, xx1, xx2); %assume back/forth traj linearization reversed
    ...
    for (tt = 1:numel(sessevpairs)) %%%stability
        pinfo.field.run1DStablty{i}(tt) = NaN; pinfo.field.run1DStabP{i}(tt) = NaN;
        pinfo.field.run1DSlideShufStablty{i}(tt) = NaN; pinfo.field.run1DSlideShufStabltyZ{i}(tt) = NaN; 
        rate1 = []; xx1 = []; rate2 = []; xx2 = []; maa = 0; mbb = 0; mmrate1 = []; mmrate2 = [];
        ...
        if (~isempty(rate1)) && (~isempty(rate2))
           if (max(rate1) + max(rate2))>0
              ...
              for jjj = 1:nshuffle
                   scrr(jjj) = correlateratemaps(rate1, slideshuffle(rate2), xx1, xx2, 0, 0);
              end
    ...
    pinfo.field.PF1DNfield{i} = nfield; pinfo.field.PF1DBaseRate{i} = baserate; disp(['-----------> number of 1D fields identified: ', num2str(nfield)]);
end
...
function [r1, r2, x] = realignjointxx(xjoint1, xjoint2, rate1, rate2, xx1, xx2)
%%%%realign opposite trajectories assuming the center landmarks are matched
%%%%assuming rate1/rate2 xx1/xx2 are opposite trajectories
...
if i>=0
   [C,P] = corrcoef(rate1(i+1:np), rate2(1:np-i)); crossP = P(1,2);
else
   [C,P] = corrcoef(rate1(1:np+i), rate2(1-i:np)); crossP = P(1,2);
end
