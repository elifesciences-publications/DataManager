    ...
    elseif (strcmp(tagmark, 'thetaphase'))
        ...
        if (cc==1) 
            if plotparm.linkeeg == 1  %%check eeg
                ...
                msgbox('No EEG database linked; Fields not computed'); ok = 0;
            end
            if ok %%%check behav
            if plotparm.linkbehav == 1
                behav = getappdata(hf, 'behav'); bhdata = getappdata(hf, 'bhdata');
...
pp = {'Baseline session for 2D map dynamics (session name)?'; 'Baseline segment for 2D map dynamics (first, last, average, number(s))?';...
      'Baseline session for 1D map dynamics (session name)?'; 'Baseline lap for 1D map dynamics (first, last, average, number(s))?'};
def = {'Open1'; 'last'; 'linearrun'; 'last'};
...
if (~isempty(III))
   ...
   for (i = 1:ncell) pinfo.parm.fd2DBaseSess{cellind(i)} = III{1}; end
   for (i = 1:ncell) pinfo.parm.fd2DBaseSeg{cellind(i)} = III{2}; end
   for (i = 1:ncell) pinfo.parm.fd1DBaseSess{cellind(i)} = III{3}; end
   for (i = 1:ncell) pinfo.parm.fd1DBaseLap{cellind(i)} = III{4}; end
else
   ok = 0;
end
...
pp = {'Smooth 2D rate maps?'; 'Enter 2D smoothing sigma (pixel);'; 'Enter 2D N sigmas for smoothing:';...
    'Smooth 1D rate maps?'; 'Enter 1D smoothing sigma (pixel):'; 'Enter 1D N sigmas for smoothing:'};
def = {'yes'; '20'; '5'; 'yes'; '20'; '5'};
...
pp = {'Enter 1D grid N for computing baseline rate (x total grid N):'; 'Enter 1D minimum peak rate (Hz):'; ...
    'Enter 1D minimum in-field rate (x peak rate):'; 'Enter 1D maximum in-field gap (pixels):'; 'Enter 1D minimum laps'};
def = {'0.3'; '1'; '0.1'; '20'; '4'};
...
pp = {'Enter 2D grid N for computing baseline rate (x total grid N):'; 'Enter 2D minimum peak rate (Hz):'; ...
    'Enter 2D minimum in-field rate (x peak rate):'; 'Enter 2D maximum in-field gap (pixels):'};
def = {'0.3'; '3'; '0.1'; '20'};
...
if (ok)
pp = {'Enter max diretional shifts (pixels)?'; 'Correlate rotated rate maps?'; 'Enter rotating center X (pixels):';...
    'Entering rotating center Y (pixels):'; 'Enter rorating angle (degree):'};
def = {'100'; 'no'; '400'; '180'; '90'};
