...
elseif (strcmp(tagmark, 'scatter')) %%%bar plot either mean or median
   ...
   if ok
      ...
      %bar(hax, binvector, Y, 'EdgeColor', cl, 'FaceColor', cl);
      for (i = 1:n)
          line(binvector(i)+[-0.2 0.2], [Y(i) Y(i)], 'Parent', hax, 'Color', cl, 'LineWidth', 2); %
      end
%       for (i = 1:n)
...
elseif (strcmp(tagmark, 'regression'))
   for (i = 1:n)
       ...
       for (j = i+1:n)
           ...
           line(val{i}(1:nok), val{j}(1:nok), 'Parent', hax, 'LineStyle', 'none', 'Marker', '.', 'MarkerSize', 5);
           ...
           if (std(val{i}(1:nok)) ~= 0)
              [str1, str2, YY, XX] = DoCrrRegress(val{i}(1:nok), val{j}(1:nok)); str{3} = str1; str{4} = str2;
