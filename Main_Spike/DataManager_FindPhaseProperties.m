...
if (~isempty(cellind))
   ...
   %%%%%phase precession within place fields of 1D track
...
for (jjjk = 1:numel(cellind))
    ...
    nsess = numel(sessionname); sesstype = pinfo.parm.sessType{i}; 
    ...
    for (tt = 1:nsess)
        ...
        if (numel(iii)~=1) || isempty(currenttimepoint{iii})
            ...
            if filterevents(sessionname{tt}, sesstype{tt}, parm.sesskeyword, parm.sesskeytype)
               if ~isempty(jjj)
                  P = findphaseprop(spikephase(jjj));
    ...
    for (tt = 1:numel(evName))
        if filterevents(evName{tt}, evType{tt}, parm.evtkeyword, parm.evtkeytype)
            ...
            phasenow = spikephase(spikeid); P = findphaseprop(phasenow);
    ...
    %%%%%compute phase prcession within 1D place fields
    ...
    for tt = 1:numel(fprate) %%%for every field exist on linear track
        ...
        if (numel(j)==1)&& filterevents(evName{j}, evType{j}, parm.evtkeyword, parm.evtkeytype) %%%if this is a run event on the selected traj 
            ...
            if (numel(posid)~=1)||(numel(evid)~=1)
               ...
               fstart = pinfo.field.PF1DBoundStart{i}(tt); fend = pinfo.field.PF1DBoundEnd{i}(tt); fstart = fstart+50; fend = fend-20;
               ...
               pinfo.phase.fieldMeanPhase{i}(tt) = P.meanphase; pinfo.phase.fieldPhaseVar{i}(tt) = P.phasevar;
...
P.meanphase = NaN; P.phasevar = NaN; P.raylength = NaN; P.rayp = NaN;
