...
elseif (strcmp(tagmark, 'savegroup')) %to save group list
...
elseif (strcmp(tagmark, 'outputevents')) %output time-related variables as event files to the .final\events\ directory
   ...
   spikeind = find(spikeselection == 1);
...
if strncmpi(ntype, 'Manual', 6)
   ...
   if isempty(ii)
      ...
      newgrouplist.groupindex{groupnow} = data.grouplist.groupindex{ii(1)}; 
...
for (i = 1:nfield) %search selected sub-fields in each big field
    ...
    if numel(fieldselectindex) == 2 
        Fname = fieldtitle{i}; field1 = subfield{i}{fieldselectindex(1)}; field2 = subfield{i}{fieldselectindex(2)};
        sel = questdlg(['Variables selected are: ', field1, ' and ', field2, '. Continue?']);
        if strcmp(sel, 'Yes')
...
if ok
    qq = questdlg('Filter variables through an event type?');
    if strncmpi(qq, 'Yes', 1)
       input = inputdlg({'Enter event keyword'; 'Enter event type'}, 'Event selection', 2, {'track'; 'run'}); 
       if (~isempty(input))
...
if ok
    for (i = 1:numel(ufinal))
        disp(['-------> finale dir now: ', ufinal{i}]);
        evT1 = []; evT2 = []; allsess = []; sessST = []; sessET = [];
        ...
        for (j = 1:numel(spikeind))
            if (~isempty(evkeyword)) || (~isempty(evkeytype))
                [~, ~, evTimes] = filterevents(pinfo, data, evkeyword, evkeytype, spikeind(j));
            end
            V1 = value1{spikeind(j)}; V2 = value2{spikeind(j)}; V1 = reshape(V1, numel(V1), 1); V2 = reshape(V2, numel(V2), 1);
            %[a, b] = size(V1); if (a==1) V1 = V1'; end
            %[a, b] = size(V2); if (a==1) V2 = V2'; end
            evT1 = [evT1; V1]; evT2 =[evT2; V2];
        ...
        for (j = 1:numel(allsess)) %%%for each identified session, write an event file
            if (numel(evT1) ~= numel(evT2))
                disp(['-----------> warning: identified event start/end times do not match; no event generated for ', ufinal{i}, ': ', allsess{j}]);
            else
                iii = find ((evT1>=sessST(j)) & (evT1<=sessET(j)) & (evT2>=sessST(j)) & (evT2<=sessET(j)));
                ev1now = evT1(iii); ev2now = evT2(iii);
                ev1now = sort(unique(ev1now)); ev2now = sort(unique(ev2now));%%%unique and sort
                if (~isempty(evkeyword)) || (~isempty(evkeytype))
                   [ev1now, ev2now] = filtereventtimesnow(ev1now, ev2now, evTimes);
                end
                if (numel(ev1now)~=0) && (numel(ev2now)~=0) 
                    if (numel(ev2now)==numel(ev1now)) 
                        D = ev1now-ev2now;
                        if (numel(find(D<=0)) == numel(ev1now))
                           writeevt(ufinal{i}, allsess{j}, Fname, field1, field2, ev1now, ev2now, evkeyword, evkeytype);
                        elseif (numel(find(D>=0)) == numel(ev1now))
                            writeevt(ufinal{i}, allsess{j}, Fname, field2, field1, ev2now, ev1now, evkeyword, evkeytype);
                        else
...
function writeevt(ufinal, sessname, Fname, field1, field2, ev1now, ev2now, evkeyword, evkeytype)
if isempty(evkeyword) && isempty(evkeytype)
   filename = fullfile(ufinal, 'events', strcat(sessname, '_', Fname,'_',field1, '_', field2, '.evt'));
elseif isempty(evkeyword) && (~isempty(evkeytype))
   filename = fullfile(ufinal, 'events', strcat(sessname, '_', Fname,'_',field1, '_', field2, '_', evkeytype, '.evt'));
elseif (~isempty(evkeyword)) && isempty(evkeytype)
   filename = fullfile(ufinal, 'events', strcat(sessname, '_', Fname,'_',field1, '_', field2, '_', evkeyword, '.evt'));
elseif (~isempty(evkeyword)) && (~isempty(evkeytype))
   filename = fullfile(ufinal, 'events', strcat(sessname, '_', Fname,'_',field1, '_', field2, '_', evkeyword, '_', evkeytype, '.evt')); 
end
disp(['-----------> writing events to: ', filename]);
ev.start = ev1now; ev.ent = ev2now; ev.ref = zeros(numel(ev1now), 1); 
...
evName = pinfo.general.eventname{neuronid}; evsel = zeros(1,numel(evName));
...
function [ev1now, ev2now] = filtereventtimesnow(ev1now, ev2now, evTimes)
...
ev1now = ev1now(iii); ev2now = ev2now (iii);
